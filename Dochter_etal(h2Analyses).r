#R code for the reported analyses and figure generation

library(ape); library(phytools); library(MCMCglmm);library(boot);library(readxl)
library(metafor); library(cowplot);library(plotMCMC);library(brms)
library(plyr); library(emmeans); library(dplyr);library(cowplot)

Data <- read.csv("Compiled2.csv")

Tree<-read.newick("Dochter_etal(TREE).txt")

#### 0. Clean up tree & plotting the tree ####
...
#### 1. Global Mean (Beta regression with BRMS) ####
...
summary(h2.mcmc)
posterior.mode(as.mcmc(inv.logit(x$b_Intercept)))
HPDinterval(as.mcmc(inv.logit(x$b_Intercept)))
plot(as.mcmc(inv.logit(x$b_Intercept)))

#### 2. Variance Components & Heterogeneity (Intercept Only) ####
...
s2m <- sum(Data$N-3)*100/(sum(Data$N-3)^2-sum((Data$N-3)^2)) # typical sampling error
...
#These analyses were done with a sample size (N.C) set to a small value (N=100).
#This was done to check whether inferences were sensitive to possible mis-estimation of 
#sampling variance due to sample size being reported at the wrong level. 
#The inferences are generally the same, with some greater uncertainty around some effects.
...
anova(Model3.1bML,Model3.2b)#Wild vs Domestic
anova(Model3.1bML,Model3.3b)#Field vs Lab
anova(Model3.1bML,Model3.4b)#Behavior
anova(Model3.1bML,Model3.5b)#Thermy
anova(Model3.1bML,Model3.6b)#Vert
